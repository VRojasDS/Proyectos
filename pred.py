...
import torch
from torch import nn, tensor, zeros, no_grad, float32

BASE_DIR = os.path.dirname(os.path.abspath(__file__))

HEART_MODEL = os.path.join(BASE_DIR, "heartdisease.joblib")
STOCK_MODEL = os.path.join(BASE_DIR, "stock.pth")
SCALER_PATH = os.path.join(BASE_DIR, "scaler.pkl")
CAR_MODEL = os.path.join(BASE_DIR, "car_price.joblib")
...
def stock_pred(datos):
    ...
    state_dict = torch.load(STOCK_MODEL, map_location="cpu")
    ...
    x = tensor(data_scaled, dtype=float32).unsqueeze(0)
